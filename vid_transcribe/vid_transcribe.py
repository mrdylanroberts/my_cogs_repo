...
class VidTranscribe(commands.Cog):
    ...
    async def _get_loom_transcript(self, video_id: str) -> Optional[str]:
        """Extract transcript from Loom video using web scraping."""
        try:
            # Loom transcript URL pattern
            transcript_url = f"https://www.loom.com/api/campaigns/sessions/{video_id}/transcribed_text"
            
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            
            async with self.session.get(transcript_url, headers=headers) as response:
                if response.status == 200:
                    data = await response.json()
                    if 'transcribed_text' in data:
                        return data['transcribed_text']
                        
            # Fallback: Try to get transcript from the main video page
                    ...
                    for script in scripts:
                        if script.string and 'transcript' in script.string.lower():
                            ...
                            try:
                                ...
                                transcript_match = re.search(r'"transcript":\s*"([^"]+)"', script.string)
                                if transcript_match:
                                    return transcript_match.group(1).replace('\\n', '\n')
                            except:
                                continue
